...
def run_gpt_prompt_event_emotional_impact(persona, event_description):
    """
    Rates the emotional impact of an event on a persona.
    """
    prompt = (
        f"You are modeling the emotions of {persona.name}.\n"
        f"On a scale from -10 (extremely negative) to 10 (extremely positive), "
        f"how would the following event make them feel in terms of happiness, sadness, and anger? "
        f"Only return a JSON object.\n\n"
        f"Event: '{event_description}'\n\n"
        f"JSON Response:"
    )
    
    # Use the existing request function
    response_str = ChatGPT_request(prompt)
    
    try:
        # A simple parser for the JSON output
        impact = json.loads(response_str)
        return {
            "happiness": impact.get("happiness", 0),
            "sadness": impact.get("sadness", 0),
            "anger": impact.get("anger", 0)
        }
    except Exception as e:
        print(f"WARNING: Could not parse emotional impact JSON. Error: {e}")
        return {"happiness": 0, "sadness": 0, "anger": 0}
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  def create_prompt_input(persona, wake_up_hour, test_input=None):
    ...
    if persona.scratch.weather:
        prompt_input += [persona.scratch.weather]
    # --- END NEW WOLFRAM INTEGRATION ---
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # Llama 3 often provides a verbose, multi-line response. 
    # We need to find the specific activity for the hour requested in the prompt.
    
    # First, find the timestamp from the end of the prompt.
    # e.g., "Monday February 13 -- 06:00 AM"
    prompt_lines = prompt.strip().split('\n')
    last_prompt_line = prompt_lines[-1]
    
    # Extract the timestamp, e.g., "06:00 AM"
    try:
        target_hour_str = last_prompt_line.split("--")[1].split("]")[0].strip()
    except IndexError:
        # Fallback: if we can't find the timestamp, just grab the first line of the response.
        return gpt_response.split('\n')[0].strip()

    # Now, search for that timestamp in the model's multi-line response.
    for line in gpt_response.split('\n'):
        if target_hour_str in line and "Activity:" in line:
            # We found the correct line. Extract the activity description.
            activity = line.split("Activity:")[1].strip()
            # Clean up trailing periods or other unwanted characters
            if activity.endswith("."):
                activity = activity[:-1]
            return activity
    
    # If no matching timestamp is found, it's likely the model just gave the
    # next activity. We'll try to find the first line that looks like an activity.
    for line in gpt_response.split('\n'):
        cleaned_line = line.strip()
        if "Activity:" in cleaned_line:
            activity = cleaned_line.split("Activity:")[1].strip()
            if activity.endswith("."):
                activity = activity[:-1]
            return activity
            
    # As a final fallback, return the first non-empty line of the response.
    for line in gpt_response.split('\n'):
        if line.strip():
            return line.strip()
  ...
  prompt = generate_prompt(prompt_input, prompt_template)
  fail_safe = get_fail_safe()
  # --- CHANGE: Using the modern ChatGPT function ---
# This directly fixes the "TOKEN LIMIT EXCEEDED" error.
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # This new version is designed to handle Llama's verbose and sometimes
    # improperly formatted output for the task decomposition prompt.
    import re
    
    # This regex looks for lines starting with a number, followed by the description,
    # and captures the duration in minutes. It's robust to variations in spacing
    # and extra text.
    pattern = re.compile(r"^\d+\)\s*(.*?)\s*\(duration in minutes:\s*(\d+)")
    
    matches = pattern.findall(gpt_response)
    
    cr = []
    for match in matches:
        task_description = match[0].strip()
        duration = int(match[1])
        # Ensure tasks are at least 5 minutes long and in 5-minute increments
        if duration > 0:
          final_duration = max(5, round(duration / 5) * 5)
          cr.append([task_description, final_duration])

    if not cr:
        # If the regex found no matches, it's a critical failure.
        # We'll return a default action to prevent a crash.
        print("WARNING: Could not parse any valid subtasks from the LLM response.")
        task = prompt.split("is ")[-1].split(" from")[0]
        total_duration = int(prompt.split("(total duration in minutes")[-1].split("):")[0].strip())
        return [[task, total_duration]]
    
    # This part of the code ensures the total duration of the subtasks
    # exactly matches the total time allocated for the main task.
    total_expected_min = int(prompt.split("(total duration in minutes")[-1]
                                   .split("):")[0].strip())
    
    current_total_min = sum(item[1] for item in cr)

    if cr and current_total_min != total_expected_min:
        diff = total_expected_min - current_total_min
        new_last_duration = cr[-1][1] + diff
        if new_last_duration > 0:
            cr[-1][1] = new_last_duration
        else:
            # If the adjustment makes the last task's duration non-positive,
            # we'll remove tasks from the end until it fits.
            while current_total_min > total_expected_min and cr:
                removed_task = cr.pop()
                current_total_min -= removed_task[1]
            if cr:
                 cr[-1][1] += total_expected_min - current_total_min
...
def run_gpt_prompt_action_arena(action_description, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # This new version handles cases where the LLM includes brackets or extra spaces.
    cleaned_response = gpt_response.replace("{", "").replace("}", "").strip()
    return cleaned_response
  ...
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
                                   __func_validate, __func_clean_up)
  print (output)
  valid_arenas = [arena.strip() for arena in prompt_input[2].split(',')]
  
  # Check if the model's output is actually one of the valid options.
  if output not in valid_arenas:
    print(f"WARNING: LLM hallucinated an invalid arena ('{output}').")
    print(f"         Valid options were: {valid_arenas}. Defaulting to the first valid option.")
    
    # If the output is invalid, force it to be the first valid option.
    if valid_arenas:
        output = valid_arenas[0]
    else:
        # This is a fallback in case there are no valid arenas for some reason.
        output = fail_safe
  # --- END: NEW VALIDATION BLOCK ---

  print (output) # You can remove this print statement after debugging
